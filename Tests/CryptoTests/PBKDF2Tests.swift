...
class PBKDF2Tests: XCTestCase {
    ...
    func testSHA1() throws {
        ...
        for test in tests {
            let result = try PBKDF2<SHA1>.derive(fromPassword: [UInt8](test.key.utf8), saltedWith: [UInt8](test.salt.utf8), iterating: test.iterations, derivedKeyLength: SHA1.digestSize).hexString.lowercased()
    ...
    func testMD5() throws {
        ...
        for test in tests {
            let result = try PBKDF2<MD5>.derive(fromPassword: [UInt8](test.key.utf8), saltedWith: [UInt8](test.salt.utf8), iterating: test.iterations, derivedKeyLength: MD5.digestSize).hexString.lowercased()
    ...
    func testPerformance() {
        // ~0.137 release
        measure {
            _ = try! PBKDF2<SHA1>.derive(fromPassword: [UInt8]("p".utf8), saltedWith: [UInt8]("somewhatlongsaltstringthatIwanttotest".utf8), iterating: 10_000, derivedKeyLength: SHA1.digestSize)
        }
    }
}
