...
public class RSATests: XCTestCase {
    ...
    func testDERCert() throws {
        ...
        let payloadData = parts[1].data(using: .utf8)!
        let signatureData = parts[2].data(using: .utf8)!

        let urlEncodedDer = Data(base64URLEncoded: derEncodedCer.trimmingCharacters(in: .whitespaces))!
        let signature = CryptoData.data(Data(base64URLEncoded: signatureData)!)
        let message = CryptoData.data(headerData + Data([0x2E]) + payloadData)
        let rsa = try RSAKey.public(der: CryptoData.data(urlEncodedDer))
