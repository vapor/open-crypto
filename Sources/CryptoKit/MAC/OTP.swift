...
public struct TOTP {
    ...
    public func generateRange(degree: Int, digits: OTPDigits = .six, secret: CryptoData, at date: Date = .init()) throws -> [String] {
        var res: [String] = try [
    ...
    ///               Defaults to 0.
    ///     - date: Date to generate the TOTP for. This will be divided into intervals automatically.
    public func generate(digits: OTPDigits = .six, secret: CryptoData, offset: Int = 0, at date: Date = .init()) throws -> String {
...
public struct HOTP {
    ...
    public func generate(digits: OTPDigits = .six, secret: CryptoData, counter: UInt) throws -> String {
...
private func generateOTP(secret: CryptoData, algorithm: Digest.Algorithm = .sha1, counter: UInt, digits: OTPDigits) throws -> String {
    ...
    let subdigest = [UInt8](digest[offset...offset + 3])
